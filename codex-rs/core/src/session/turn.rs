...
pub(crate) async fn run_turn(
    ...
    let post_compaction_input_tokens = estimate_turn_input_tokens(&input);
...
#[instrument(
    ...
    skip_all,
    fields(
        reason = ?compaction_metadata.reason(),
        phase = ?compaction_metadata.phase()
    )
)]
async fn run_auto_compact(
    ...
    compaction_metadata: AutoCompactionMetadata,
) -> CodexResult<()> {
